...
#Region "Private and Protected Members"
    ...
    Protected pTCPlistener As TcpListener
    Protected pNetworkStream As NetworkStream
    Protected pListenerThread As Thread
...
#Region "Constructors (Singleton Pattern)"
    ...
    'Public Shared Function GetInstance() As TCPserverFacade
    ...
    '    Friend  ReadOnly INSTANCE As TCPserverFacade = New TCPserverFacade
    'End Class
#End Region

#Region "Constructors"
    Protected Sub New()
        pIOType = BartelsLibrary.IOType.TCPserver
        pIOSettingsFacadeTemplate = SettingsFacadeTemplate.GetInstance.Build(IPsettings.GetInstance, GetType(IPsettings).Name)
    End Sub

    Public Shared Function GetInstance() As TCPserverFacade
        Return New TCPserverFacade
    End Function
#End Region
...
#Region "Public and Friend Methods"
    ...
    Public Overrides Function Open() As Boolean
        ...
        Try
            ...
            pListenerThread = New Thread(AddressOf StartListening)
            pListenerThread.Name = "TCPserver"
    ...
    Public Overrides Sub Close()
        ...
        Try
            ...
            If pTCPlistener IsNot Nothing Then
                pTCPlistener.Stop()
                pTCPlistener = Nothing
            End If
            GC.Collect()
        Catch ex As Exception
        End Try
...
#Region "Private and Protected Methods"
    Private Sub StartListening()
        Try
            ...
            pTCPlistener = New TcpListener(ipAddress, CType(pIOSettingsFacadeTemplate.ISettings, IPsettings).Port)
            pTCPlistener.Start()

            While pTCPlistener IsNot Nothing
                ' AcceptTcpclient blocks: throws exception in debug
                Dim tcpClient As TcpClient = pTCPlistener.AcceptTcpClient()
                pNetworkStream = tcpClient.GetStream()
                NotifyStatusObservers("Connected.")

                ' Read blocks
                Dim bytesRead As Int32 = pNetworkStream.Read(pBuffer, 0, pBuffer.Length)
                ' if bytesRead < 0 then while loop is exited
                While bytesRead > 0
